...
# 		die "NOR POP" . $pref{"pop_1"} . " OR " . $pref{"pop_2"} . " CONFIGURATED\n";
		my $error = "NOR POP" . $pref{"pop_1"} . " OR " . $pref{"pop_2"} . " CONFIGURATED\n";
		return (0,$error);
	}	
	elsif ($provider eq "1")
	{
...
# 	$pop->Connect() >= 0 || die $pop->Message();
	$pop->Connect() >= 0 || return(0, $pop->Message());
	...
	if ($msg_count > 0)
	{
		...
		# 		$pop->HeadAndBody( $i );
				...
				if ($line =~ m/^From:/) 
				{
					...
					{
						die "WRONG FORMATED FROM FIELD\n";
						my $error = "WRONG FORMATED FROM FIELD\n";
						return (0,$error);
					}
				}
