...
# 		die "NOR SMTP " . $pref{"pop_1"} . " OR " . $pref{"pop_2"} . " CONFIGURATED\n";
		my $error = "NOR SMTP " . $pref{"pop_1"} . " OR " . $pref{"pop_2"} . " CONFIGURATED\n";
		return (0, $error);
	}	
	elsif ($server eq "1")
	{
...
# 		die "Could not connect to server\n";
		my $error = "Could not connect to server\n";
		return (0, $error);
	}
...
# 	$smtp->auth($smtp_user, $smtp_pass) || die "Authentication failed!\n";
	$smtp->auth($smtp_user, $smtp_pass) || return (0, "Authentication failed!\n");
