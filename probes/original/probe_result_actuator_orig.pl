...
#############################
&loadconf::checkNeeds(
"doAnalyzeFirstNWBlastGlobal",	"doAnalyzeFirstNWBlast",	"doAnalyzeFirstAlmostBlast",
"doAnalyzeFirstComplexity",		"doAnalyzeFirstBlat",		"doAnalyzeFirstQuasiBlast",
...
"filterColums",					"nwColumns",				"almostColumns",
"complexityColumns",			"blatColumns",				"quasiColumns",
"translateColumns",				"doCreateTmpTable");

die "NO ARGUMENTS " if (@ARGV < 3);

my $reusePref           = $pref{"reuse"}; 			# table to reuse. undef to create a new table
my $reuseIn             = $ARGV[0];
my $PART				= $ARGV[1];
my $SUB					= $ARGV[2];
my $reuse;
my $timeStamp;
die "NO REUSE DEFINED" if ( ! defined $reuseIn);
die "NO PART  DEFINED" if ( ! defined $PART);
die "NO SUB   DEFINED" if ( ! defined $SUB);
...
if ($reuseIn =~ /^NEW(.*)/)
{
	$reuse     = 0;
	$timeStamp = $1;
	print "CREATING NEW TMP TABLE $timeStamp\n";
}
else
{
	$reuse     = 1;
...
print "TIMESTAMP $timeStamp\n";
	...
	my $blatFolder            = $pref{"blatFolder"};
	my $blat_min_identity     = $pref{"blat_min_identity"};
	my $blat_min_similarity   = $pref{"blat_min_similarity"};
	...
	$doAnalyzeFirstQuasiBlast	 = 0;
	$doAnalyzeFirstBlat			 = 0;
	$doAnalyzeFirstComplexity	 = 0;
	$doAnalyzeFirstAlmostBlast	 = 0;
	$doAnalyzeFirstNWBlast		 = 0;
	$doAnalyzeFirstNWBlastGlobal = 0;
	$doCreateFinalTable          = 0;
	$doTranslateFinalTable       = 0;
	$doAnalyzeExtra				 = 0;

	if ($SUB eq "quasi")
	{
		$doAnalyzeFirstQuasiBlast	 = 1;
	}
	elsif ($SUB eq "blat")
	{
		$doAnalyzeFirstBlat			 = 1;
	}
	elsif ($SUB eq "complexity")
	{
		$doAnalyzeFirstComplexity	 = 1;
	}
	elsif ($SUB eq "almost")
	{
		$doAnalyzeFirstAlmostBlast	 = 1;
	}
	elsif ($SUB eq "NW")
	{
		$doAnalyzeFirstNWBlast		 = 1;
	}
	elsif ($SUB eq "NWGlobal")
	{
		$doAnalyzeFirstNWBlastGlobal = 1;
	}
	elsif ($SUB eq "create")
	{
		$doCreateFinalTable          = 1;
	}
	elsif ($SUB eq "translate")
	{
		$doTranslateFinalTable       = 1;
	}
	elsif ($SUB eq "extra")
	{
		$doAnalyzeExtra              = 1;
	}
	elsif (($SUB =~ /^NEW/) && ($PART eq "PRE"))
	{

	}
	elsif (($SUB =~ /^\d+$/) && ($PART eq "PRE"))
	{

	}
	else
	{
		die "NO VALID SUB SELECTION";
	}
	...
	my %filterColums      = &splitToHash($pref{"filterColums"});		# COLUMNS TO BE USED AT INTERMEDIATE TABLE
	my %nwColumns         = &splitToHash($pref{"nwColumns"});			# COLUMNS TO BE ANALIZED BY NW
	my %almostColumns     = &splitToHash($pref{"almostColumns"});		# COLUMNS TO BE ANALIZED BY ALMOST BLAST
	my %complexityColumns = &splitToHash($pref{"complexityColumns"});	# COLUMNS TO BE ANALIZED BY COMPLEXITY
	my %blatColumns       = &splitToHash($pref{"blatColumns"});			# COLUMNS TO BE ANALIZED BY BLAT
	my %quasiColumns      = &splitToHash($pref{"quasiColumns"});		# COLUMNS TO BE ANALIZED BY QUASI BLAST
	my %translateColumns  = &splitToHash($pref{"translateColumns"});	# COLUMNS TO BE TRANSLATED
...
#############################
## DECLARE MAIN TABLES
my $FROM_ORIGINAL_VIEW     = "\`$database\`.\`$originalView\`";
my $FROM_ORIGINAL_TABLE    = "\`$database\`.\`$originalTable\`";
my $FROM_FINAL_TABLE       = "\`$database\`.\`$finalTable\`";
my $FROM_FINAL_TABLE_TRANS = "\`$database\`.\`$finalTable\_translated\`";
my $FROM_FINAL_TABLE_FINAL = "\`$database\`.\`$finalTable\_FINAL\`";

## DROP TABLES COMMANDS
my $commandDropFinalTable      = "DROP TABLE IF EXISTS $FROM_FINAL_TABLE";
my $commandDropFinalTableTrans = "DROP TABLE IF EXISTS $FROM_FINAL_TABLE_TRANS";
my $commandDropFinalTableFinal = "DROP TABLE IF EXISTS $FROM_FINAL_TABLE_FINAL";
...
my $selectOriginalTable       = "SELECT * FROM $FROM_ORIGINAL_TABLE"; # SQL STATEMENT TO SELECT ALL RESULTS FROM INTERMEDIATE TABLE. GLOBAL
my $updateOrignalTable        = "UPDATE $FROM_ORIGINAL_TABLE";        # SQL STATEMENT TO UPDATE INTERMEDIATE TABLE
...
else
{
	...
	my $createQuery  = "SELECT * FROM ";
	   $createQuery .= " (SELECT * FROM " x (scalar(keys %filterColums) - 1);
	   $createQuery .= " (SELECT " . join(", ", (sort keys %filterColums));
	   $createQuery .= " FROM $FROM_ORIGINAL_VIEW ";

	foreach my $filter (sort keys %filterColums)
	{
		$createQuery .= "GROUP BY $filter HAVING count($filter) = 1) AS t$filter ";
	}

my $commandCreateTable = "CREATE TABLE $FROM_ORIGINAL_TABLE ENGINE InnoDB $createQuery";				# SQL STATEMENT TO CREATE INTERMEDIATE TABLE
my $commandAddColumn   = "ALTER  TABLE $FROM_ORIGINAL_TABLE"                    .	# SQL STATEMENT TO ADD NEW COLUMNS
...
my $oldColumnsStr = join(", ", (sort keys %filterColums)) . ", Id ";

my $commandCreateFinalTable       = "CREATE TABLE "          . $FROM_FINAL_TABLE;
   ...
   $commandCreateFinalTable      .= " FROM "                 . $FROM_ORIGINAL_TABLE;											# FROM INTERMEDIARY TABLE
...
my $commandCreateFinalTableFinal  =  "CREATE TABLE $FROM_FINAL_TABLE_FINAL SELECT ";

my $hole_out;
foreach my $o (sort keys %outputColums)
{
	$hole_out .= ", " if ( defined $hole_out );
	$hole_out .= "\`$originalTable\`.\`$o\`";
}

my $hole_filter;
foreach my $n (sort keys %filterColums)
{
	$hole_filter .= ", " if ( defined $hole_filter );
	$hole_filter .=  "\`$finalTable\_translated\`.\`$n\`";
}
$commandCreateFinalTableFinal .= $hole_out . ", " . $hole_filter;
$commandCreateFinalTableFinal .= " FROM $FROM_ORIGINAL_TABLE, $FROM_FINAL_TABLE_TRANS";
$commandCreateFinalTableFinal .= " WHERE \`$originalTable\`.\`Id\` = \`$finalTable\_translated\`.\`Id\`";
$commandCreateFinalTableFinal .= " ORDER BY " . $orderBy if ( defined $orderBy );


my $commandCreateFinalTableTrans   = "CREATE TABLE $FROM_FINAL_TABLE_TRANS SELECT $star FROM $FROM_FINAL_TABLE";
my $commandFinalTableIndex         =	"ALTER  TABLE $FROM_FINAL_TABLE_TRANS ADD PRIMARY KEY (Id)"; 

my $commandGetAllFinalResults      = "SELECT $star FROM $FROM_FINAL_TABLE";
my $commandGetAllFinalTransResults = "SELECT $star FROM $FROM_FINAL_TABLE_TRANS";
#my $commandUpdateFinalTableCopy   = "UPDATE $FROM_FINAL_TABLE_COPY SET ligant = ?, sequence = ?, sequenceLig = ?, sequenceM13 = ? WHERE Id = ?";

#my $commandUpdateFinalTableCopy   = "UPDATE $FROM_FINAL_TABLE_COPY SET ligant = \'<0>\', sequence = \'<1>\', sequenceLig = \'<2>\', sequenceM13 = \'<3>\' WHERE Id = \'<4>\'";
my $commandUpdateFinalTableTrans   = "UPDATE $FROM_FINAL_TABLE_TRANS SET ";

my $commandCheckFinalTable         = "SELECT $star FROM $FROM_FINAL_TABLE";
...
#my $commandAddPrimaryKey     = "ALTER  TABLE         $FROM_FINAL_TABLE  ADD PRIMARY KEY (Id)"; 
...
if ($doCreateFinalTable)
{
	&sthInsertCreate($commandDropFinalTable, $commandCreateFinalTable);
}

if ($doTranslateFinalTable)
{
	&sthInsertCreate($commandDropFinalTableTrans, $commandCreateFinalTableTrans, $commandFinalTableIndex);
	&sthAnalizeStat($commandGetAllFinalTransResults);
	&sthTranslateFinalTable($commandGetAllFinalTransResults, $commandUpdateFinalTableTrans);
	&sthInsertCreate($commandDropFinalTableFinal, $commandCreateFinalTableFinal);
}
...
sub sthTranslateFinalTable()
{
	...
	while(my $row = $sth1->fetchrow_arrayref) 
	{
		...
		my @cols;
		foreach my $keys (sort keys %filterColums) {$cols[$filterColums{$keys}] = $keys };

		foreach my $colum (sort keys %filterColums)
		{
			my $index = $filterColums{$colum};
...
sub sthAnalizeStat
{
	...
	if ($statVerbose)
	{
		...
		{
			printf "\t\t%-" . $maxLen . "s COLUMN INDEX = %02d", uc($col), $oldColumnIndex{$col};

			if (exists $filterColums{$col})
			{
				print " (*)";
				$filterColums{$col} = $oldColumnIndex{$col};
			}
			else
			{
				print " (-)";
				$outputColums{$col} = $oldColumnIndex{$col};
			}
		...
		{
			next if ( ! defined $newColumnIndex{$col} );
...
#				print "START: $start END: $end FRACTION: $fraction PARTS: $parts TOTAL: ",$arraySize, "\n";
	...
	sub sthAnalizeSimilarity5
	{
		...
		#TODO: SOFTCODE
		my $columnIndexId     = $oldColumnIndex{"Id"};
		...
		my $countKey = 0;
		foreach my $key (sort keys %filterColums)
		{
			#TODO: DONT SKIP THIS WAY
			next if $key eq "sequenceLig";
			next if $key eq "sequenceM13";
			my $index = $filterColums{$key};
			$files[$index][0] = "$blatFolder/Q_query_"  . $key . ".fa";
			$files[$index][1] = "$blatFolder/Q_db_"     . $key . ".fa";
			$files[$index][2] = "$blatFolder/Q_result_" . $key . ".fa";
			$files[$index][3] = 0;
			$files[$index][4] = 300;
			$countKey++;
		}
		...
		my $retT = time;
		print "_"x6, "SIMILARITY 5 :: RETRIEVING VALID RESULTS\n";
		my $sth5V = $dbh5->prepare($commandGetValid);
		$sth5V->execute();
		my $rowsV = $sth5V->rows;
		print "_"x6, "SIMILARITY 5 :: ",$rowsV, " VALID RESULTS RETRIEVED SUCCESSIFULLY IN " , (time - $retT) , "s\n";

		while(my $row = $sth5V->fetchrow_arrayref) 
		{
			$countRow++;
			print "_"x4, "SIMILARITY 5 :: EXPORTING VALID ROW # $countRow\n" if ( ! ($countRow % (int($rowsV / 5))));
			...
			foreach my $key (sort keys %filterColums)
			{
				...
				my $leng          = length($seq);
				$files[$index][3] = $leng if (( ! defined $files[$index][3]) || ($leng > $files[$index][3]));
				$files[$index][4] = $leng if (( ! defined $files[$index][3]) || ($leng < $files[$index][4]));
				print { $fhs[$index][0] } ">", $rowNum , "|" , $rowNum , "_" , $rowNum , "\n" , $transSeq, "\n\n";
			}
			$fileCount++;
		}
		...
		print "_"x6, "SIMILARITY 5 :: ",$rowsV, " VALID DB RESULTS EXPORTED SUCCESSIFULLY IN " , (time - $retT) , "s\n";
		...
		$retT = time;
		print "_"x6, "SIMILARITY 5 :: RETRIEVING ALL VALID RESULTS\n";
		my $sth5A = $dbh5->prepare($commandGetAll);
		$sth5A->execute();
		my $rowsA = $sth5A->rows;
		print "_"x6, "SIMILARITY 5 :: ",$rowsA, " ALL VALID RESULTS RETRIEVED SUCCESSIFULLY IN " , (time - $retT) , "s\n";
		...
		{
			$countRow++;
			print "_"x4, "SIMILARITY 5 :: EXPORTING ALL ROW # $countRow\n" if ( ! ($countRow % (int($rowsV / 5))));
			my $rowNum = $row->[$columnIndexId];
			...
			foreach my $key (sort keys %filterColums)
			{
				...
				print { $fhs[$index][1] } ">", $rowNum , "|" , $rowNum , "_" , $rowNum , "\n" , $transSeq, "\n\n";
			}
			$fileCount++;
		}
		$sth5A->finish();
		...
		print "_"x6, "SIMILARITY 5 :: ",$rowsA, " ALL DB RESULTS EXPORTED SUCCESSIFULLY IN " , (time - $retT) , "s\n";
		...
		foreach my $fh (@fhs)
		{
			close $fhs[$fh][0] or die "COULD NOT CLOSE FILEHANDLE";
			close $fhs[$fh][1] or die "COULD NOT CLOSE FILEHANDLE";
		}
		...
		#####################
		#&analizeSimilarity5($commandUpdate, \@files);
		#####################
	...
	sub analizeSimilarity5
	{
		...
		for (my $f = 0; $f < @files; $f++)
		{
			my $queryFile  = $files[$f][0];
			my $dbFile     = $files[$f][1];
			my $resultFile = $files[$f][2];
			my $maxSize    = $files[$f][3];
			my $minSize    = $files[$f][4];

			my $retT = time;
			print "_"x6, "SIMILARITY 5 :: RUNNING BLAT\n";
			#blat/filterpsl.sh blat blat/3_db.fa blat/3_query.fa blat/3_result 20
			################
			my $blatCMD = "$blatFolder/filterpsl.sh $blatFolder $dbFile $queryFile $resultFile $minSize $blat_min_identity $blat_min_similarity";
			print "_"x6, $blatCMD, "\n";
			print `$blatCMD`;
			################
			print "_"x6, "SIMILARITY 5 :: BLAT RUNNED IN " . (time - $retT) . "\n";
			...
			print "_"x6, "SIMILARITY 5 :: BLAT RESULT LOADED IN " . (time - $retT) . "\n";
	...
	sub sthAnalizeSimilarity5_old
	{
		...
		my $columnIndexSeq    = $oldColumnIndex{"sequence"};
		my $columnIndexLigant = $oldColumnIndex{"ligant"};
		my $columnIndexId     = $oldColumnIndex{"Id"};
