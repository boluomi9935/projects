...
#############################################
&loadconf::checkNeeds(
"actuatorDelete",	"maxGCLS",		"log",
"maxThreads",		"napTime",		"cleverness",
"verbose",			"insertSize",	"max_packet_size",
"exportSeq",		"ligLen",		"ligMinGc",
"ligMaxGc",			"ligMinTm",		"ligMaxTm",
"m13Len",			"m13MinGc",		"m13MaxGc",
"m13MinTm",			"m13MaxTm",		"primerFWD",
"primerREV",		"database");
...
#TODO: ADD 10% TO GC AND TM
my $ligLenStr 		= $pref{'ligLen'};
my @ligLen;
			...
			while ($ligStart < $lastLigStart)
			{
				...
				for $ligLen (@ligLen)
				{
					...
					for $m13Len (@m13Len)
					{
						...
						if (( $m13Tm > $ligTm ) && ( ($m13Tm - $ligTm) > 5))                   { next; };
						if (( $ligTm > $m13Tm ) && ( ($ligTm - $m13Tm) > 5))                   { next; };
