...
sub countAll
{
	...
	my $listProbe       = $dbh->prepare_cached("SELECT COUNT(*) as TotalProbes      FROM probe");
	my $listCoordinates = $dbh->prepare_cached("SELECT COUNT(*) as TotalCoordinates FROM coordinates");
