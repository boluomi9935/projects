...
#############################
	...
	$vars{commandGetAllResults}    = "$selectIntermediateTable";	# SQL STATEMENT TO GET ALL RESULTS
	...
	{
		&sthAnalizeStat($vars{commandGetAllResults} . " LIMIT 1");	# get result from intermediate table
        ...
        if ($name eq $vars{SUB}) # for thread
		{
			...
			if ( ! $dryRun )
			{
				$sql{$name}{function}->(
					...
					$vars{commandGetAllResults},					# THE ALL RESULTS SQL STATEMENT
