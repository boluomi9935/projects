#! /usr/bin/perl -w
#http://www.perlmonks.org/?node_id=308579
#
...
sub BuildString {
    ...
    die "Can't currently handle more this many strings (",(256-INVALID_BASE())," max)" if
        @_ >= 256-INVALID_BASE();
    $txt = "";
    my $chr = INVALID_BASE;
    $want_map = "";
...
if (@ARGV) {
    ...
    if ( @ARGV == 1 )
    {
        if ( -f $ARGV[0] )
        {
            print "LOADING FILE $ARGV[0]\n";

            open NFO, "<$ARGV[0]" or die "COULD NOT OPEN $ARGV[0] : $!";
            @lines = <NFO>;
            close NFO;
            print "\t",scalar @lines," LINES READ\n";
            if ( @lines < 2 ) { print "NO LINES READ\n"; exit 1; };
        } else {
            print "ONLY 1 ARGUMENT GIVEN AND IT'S NOT A FILE : $ARGV[0]\n";
            exit 1;
        }
    } else {
        print "LOADING NAMES :", @ARGV, "\n";
        @lines = @ARGV;
    }
    ...
    if (1) {
        ...
        print "LCS=<", CommonSubstring(@lines), ">\n";
    }
} else {
    # Default strings for basic sanity checking and demo
    BuildString( qw/aba abba abbda abbaracadabra bam bamara barbara saulo/ );
